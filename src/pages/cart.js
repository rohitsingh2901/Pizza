    ...
    <div className="container mx-auto mt-12">
        ...
        <ul> 
          <li><h1 className="my-12 mx-32 font-bold ">Cart Items</h1></li>                
          {
            Product.map(product => {
              return(
              <li key={product._id.$oid} className="flex justify-around items-center mb-6">
                <div className="flex">
                <img className="h-16 w-16" src={product.image} alt="pizzaImage"></img>
                <h1 className="font-bold ml-4 w-48">{product.name}</h1>
                </div>
     ...
     :
     <>
     <img className="container mx-auto h-96 w-96 mt-20" src="/empty-cart.png" alt="cart"></img>
     <div className="container mx-auto text-center mt-12">
      <h4><b>Let's fill the empty Cart</b></h4>
      <Link to='/'><button className='px-5 py-2 my-3 w-fit rounded-full text-xs  font-bold bg-yellow-500  hover:bg-yellow-700'>Explore Pizzas</button></Link>
     </div>
     </>
  )
}
